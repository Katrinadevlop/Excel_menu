...
class MenuTemplateFiller:
    ...
    def copy_kassa_rect_A6_F42(self, template_path: str, source_menu_path: str, output_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Вставляем салаты A28-A41 (максимум 14 позиций)
            for i, (name, weight, price) in enumerate(salads[:14]):
                row = 28 + i
                try:
                    tpl_ws.cell(row=row, column=1).value = name
                    tpl_ws.cell(row=row, column=2).value = weight
            ...
            try:
                # A28 теперь используется для салатов (заголовок уже скопирован из источника)
